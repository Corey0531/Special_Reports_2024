...
const int xPin = A0;  //X軸 接類比A0
const int yPin = A1;  //Y軸 接類比A1
const int zPin = A2;  //Z軸 接類比A2
...
// 讀取搖桿的函式
void readVR() {
    xval = analogRead(xPin); // 讀取 X 軸數值
    yval = analogRead(yPin); // 讀取 Y 軸數值
    zval = analogRead(zPin); // 讀取 Z 軸數值
